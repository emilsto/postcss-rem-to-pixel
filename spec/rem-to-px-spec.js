...
describe('remToPx', function () {
    it('should work on the readme example', function () {
        const input = 'h1 { margin: 0 0 20px; font-size: 2rem; line-height: 1.2; letter-spacing: 0.0625rem; }';
        const output = 'h1 { margin: 0 0 20px; font-size: 32px; line-height: 1.2; letter-spacing: 1px; }';
...
describe('value parsing', function () {
    it('should not replace values in double quotes or single quotes', function () {
        ...
        const expected = '.rule { content: \'1rem\'; font-family: "1rem"; font-size: 16px; }';
        const processed = postcss(remToPx(options)).process(rules).css;
    ...
    it('should not replace values with an uppercase R or REM', function () {
        ...
        const rules = '.rule { margin: 0.75rem calc(100% - 14REM); height: calc(100% - 1.25rem); font-size: 12Rem; line-height: 1rem; }';
        const expected = '.rule { margin: 12px calc(100% - 14REM); height: calc(100% - 20px); font-size: 12Rem; line-height: 16px; }';
        const processed = postcss(remToPx(options)).process(rules).css;
...
describe('propList', function () {
    ...
    it('should only replace properties in the prop list', function () {
        const rules    = '.rule { font-size: 1rem; margin: 1rem; margin-left: 0.5rem; padding: 0.5rem; padding-right: 1rem }';
        const expected = '.rule { font-size: 16px; margin: 16px; margin-left: 0.5rem; padding: 0.5rem; padding-right: 16px }';
        const options = {
    ...
    it('should only replace properties in the prop list with wildcard', function () {
        const rules    = '.rule { font-size: 1rem; margin: 1rem; margin-left: 0.5rem; padding: 0.5rem; padding-right: 1rem }';
        const expected = '.rule { font-size: 1rem; margin: 16px; margin-left: 0.5rem; padding: 0.5rem; padding-right: 1rem }';
        const options = {
...
describe('selectorBlackList', function () {
    ...
    it('should ignore every selector with `body$`', function () {
        const rules = 'body { font-size: 1rem; } .class-body$ { font-size: 16rem; } .simple-class { font-size: 1rem; }';
        const expected = 'body { font-size: 16px; } .class-body$ { font-size: 16rem; } .simple-class { font-size: 16px; }';
        const options = {
    ...
    it('should only ignore exactly `body`', function () {
        const rules = 'body { font-size: 16rem; } .class-body { font-size: 1rem; } .simple-class { font-size: 1rem; }';
        const expected = 'body { font-size: 16rem; } .class-body { font-size: 16px; } .simple-class { font-size: 16px; }';
        const options = {
            selectorBlackList: [/^body$/]
        };
        const processed = postcss(remToPx(options)).process(rules).css;
...
describe('minRemValue', function () {
    it('should not replace values below minRemValue', function () {
        const rules = '.rule { border: 0.0625rem solid #000; font-size: 1rem; margin: 0.0625rem 0.625rem; }';
        const expected = '.rule { border: 0.0625rem solid #000; font-size: 16px; margin: 0.0625rem 10px; }';
        const options = {
            propList: ['*'],
            minRemValue: 0.5
        };
...
describe('pxToRem', function () {
    it('should convert from px to rem and back using postcss-pxtorem', function () {
        const input = 'h1 { margin: 0 0 20px 0.5rem; font-size: 13px; line-height: 1.2; letter-spacing: 1px; }';
